...
const ExclusiveBlock: React.FC<Props> = ({
  ...
  return (
    <div className={`${containerClasses[containerWidth]} ${className}`}>
      <div className={`rounded-md overflow-hidden lg:block relative ${heightClasses[height]}`}>
        {overlayEnabled && (
          <div
        ...
        <div className={`flex justify-between ${gapClasses[gridGap]}`}>
          {data?.exclusiveData?.slice(0, 2).map((item: any) => (
            <div
              className={`group w-2/4 flex justify-between items-end relative transition duration-200 ease-in ${item.id === 2
            ...
            >
              ...
              {data.exclusiveName && (
                <div
                  className={`z-0 absolute top-10 xl:top-12 2xl:top-16 3xl:top-24 uppercase text-black opacity-10 text-xl xl:text-2xl 3xl:text-3xl tracking-widest leading-5 ${item.id === 2
              ...
              {data.year && (
                <div
                  className={`exclusiveYear absolute top-16 xl:top-20 2xl:top-24 3xl:top-32 ltr:left-0 rtl:right-0 z-10 text-black font-bold leading-none tracking-widest ${item.id === 2
